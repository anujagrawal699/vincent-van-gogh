...
export function useActivitySuggestions() {
  ...
  const filtered = useMemo(() => {
    ...
    return state.activities.filter((a) => {
      ...
      const matchesCat =
        ...
        state.filters.categories.includes(a.category);
      const matchesEnergy =
        state.filters.energy === "any" || a.energy === state.filters.energy;
      const matchesTime =
        state.filters.timeOfDay === "any" ||
        a.timeOfDay === state.filters.timeOfDay ||
        a.timeOfDay === "any";
  ...
  const suggestions: Activity[] = useMemo(() => {
    // No suggestions if any filter is active or theme is default/null
    const hasActiveFilters =
      state.filters.query !== "" ||
      state.filters.categories.length > 0 ||
      state.filters.energy !== "any" ||
      state.filters.timeOfDay !== "any";
    const isDefaultOrNoTheme =
      !state.selectedTheme || state.selectedTheme.id === "default";
