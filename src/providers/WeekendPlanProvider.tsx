...
export function WeekendPlanProvider({ children }: { children: React.ReactNode }) {
  ...
  const conflicts = useMemo(() => TimeSlotService.detectConflicts([...state.schedule.saturday, ...state.schedule.sunday]), [state.schedule]);
  const value = useMemo(() => ({ state, dispatch, conflicts }), [state, conflicts]);
  return (<WeekendPlanContext.Provider value={value}>{children}</WeekendPlanContext.Provider>);
}
