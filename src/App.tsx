...
function AppShell() {
  ...
  type SlotDropData = { type: 'slot'; day: 'saturday' | 'sunday'; slot: 'morning' | 'afternoon' | 'evening' | 'night' };
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    if (aData?.type === 'activity' && oData?.type === 'slot') {
      ...
      dispatch({ type: 'add', payload: { activity, day: oData.day, slot: oData.slot, durationHours: activity.duration } });
    }
  };
      ...
      <header className="sticky top-0 z-10 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-2 px-3 sm:px-4 py-3">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-semibold">Weekendly - Van Gogh Edition</h1>
              {state.selectedTheme && (
                <div className="text-xs text-gray-500 hidden sm:block">
                  {state.selectedTheme.mood} weekend • {state.selectedTheme.vanGoghPainting}
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button className="rounded-lg border border-gray-700 hover:bg-gray-100 px-2 py-1 text-xs sm:px-3 sm:text-sm" onClick={() => dispatch({ type: 'randomize' })}>Surprise Me</button>
            <button className="rounded-lg border border-gray-700 hover:bg-gray-100 px-2 py-1 text-xs sm:px-3 sm:text-sm" onClick={() => dispatch({ type: 'clear' })}>Clear</button>
            <ShareModal />
          </div>
        </div>
      </header>
        ...
        <div className="block lg:hidden space-y-6">
          ...
          <section>
            ...
            <div className="block sm:hidden text-sm text-gray-600 p-4 bg-blue-50 rounded-lg">
              💡 Tap the <strong>+</strong> buttons above to add activities to your schedule
            </div>
          </section>
        </div>
