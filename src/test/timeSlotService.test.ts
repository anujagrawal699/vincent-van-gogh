...
describe('TimeSlotService', () => {
  ...
  describe('getTotalDurationInSlot', () => {
    it('calculates total duration for activities in a slot', () => {
      ...
      const total = TimeSlotService.getTotalDurationInSlot(activities, 'morning');
      expect(total).toBe(3);
    });
  });
  ...
  describe('isSlotOverCapacity', () => {
    it('returns true when slot is over capacity', () => {
      ...
      expect(TimeSlotService.isSlotOverCapacity(activities, 'morning')).toBe(true);
    });
    ...
    it('returns false when slot is at or under capacity', () => {
      ...
      expect(TimeSlotService.isSlotOverCapacity(activities, 'morning')).toBe(false);
    });
  });
});
