    ...
    <div className="flex flex-wrap gap-2">
      {Object.keys(categoryLabels).map(cat => (
        <button key={cat}
          ...
          className={`rounded-full border px-3 py-1 text-sm bg-white ${selected.includes(cat as Category) ? 'bg-gray-200 border-gray-900' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
        >{categoryLabels[cat as Category]}</button>
      ))}
    </div>
  );
}
